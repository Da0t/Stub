...
// Hozier set on Lands End (from data/grid.sample.json)
const HOZIER = GRID.sets.find((s) => s.artistName === 'Hozier')!;
const midHozier = (HOZIER.startTime + HOZIER.endTime) / 2;
...
test('lookupSet: mid-set returns the running set', () => {
  assert.equal(lookupSet('lands-end', midHozier, GRID), HOZIER.id);
});

test('lookupSet: between sets returns null', () => {
  const between = 1786240000000; // in the gap between the opener and Hozier on Lands End
  assert.equal(lookupSet('lands-end', between, GRID), null);
});

test('lookupSet: 10 min before start counts (grace window)', () => {
  const tenBefore = HOZIER.startTime - 10 * 60_000;
  assert.equal(lookupSet('lands-end', tenBefore, GRID), HOZIER.id);
});

test('lookupSet: 20 min before start does not count', () => {
  const twentyBefore = HOZIER.startTime - 20 * 60_000;
  assert.equal(lookupSet('lands-end', twentyBefore, GRID), null);
});

test('lookupSet: after the last set returns null', () => {
  const afterAll = HOZIER.endTime + 60 * 60_000;
...
test('lookupSet: unknown stage returns null', () => {
  assert.equal(lookupSet('no-such-stage', midHozier, GRID), null);
});

// ---- resolve (combined) ---------------------------------------------------

test('resolve: Lands End centroid at mid-Hozier returns the Hozier set (offline)', () => {
  const r = resolve(landsEndCentroid, midHozier, GRID);
  assert.deepEqual(r, { stageId: 'lands-end', setId: HOZIER.id });
});

test('resolve: an off-stage point is {null, null} at any time', () => {
  const wineLands: LatLng = { lat: 37.76645, lng: -122.4933 };
  assert.deepEqual(resolve(wineLands, midHozier, GRID), { stageId: null, setId: null });
...
test('resolve: at a stage but between sets gives a stage with a null set', () => {
  const r = resolve(landsEndCentroid, 1786240000000, GRID);
  assert.deepEqual(r, { stageId: 'lands-end', setId: null });
});

test('resolve: is deterministic — two runs on the same input agree', () => {
  const r1 = resolve(landsEndCentroid, midHozier, GRID);
  const r2 = resolve(landsEndCentroid, midHozier, GRID);
